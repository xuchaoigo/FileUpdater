...
VOID FileDiff::listDir(PCSTR dir, UINT depth, BOOL old_or_new)
{
    DIR *dp;
    struct dirent *entry;
    struct stat statbuf;
...
std::string FileDiff::calcMD5(std::string full_path)
{
    ...
    INT len;
    UCHAR *pData = (UCHAR*)malloc(1024*1024*1024);
    if(!pData)
    {
...
*/
    ...
    for(it = add_set.begin(); it != add_set.end(); it++)
	{
		...
		cJSON_AddStringToObject(pfile, "md5","xxxx");
	}
